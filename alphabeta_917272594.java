import java.util.*;
public class alphabeta_917272594 extends AIModule
{
	...
	{   //makes a copy of the current game board
        ...
        //calls MiniMax
        int [] move = MiniMax(game, currentBoard, 2, -999999999, 999999999, game.getActivePlayer());
        //System.out.println("Best Col is " + move[1]);
        chosenMove = move[1];
	}
    //MiniMax Returs array: [0] = score,  [1] = column
    private int [] MiniMax (final GameStateModule game, int [][] board, int depth, int alpha, int beta, int Player){
        int [] validLoc = validMove(game, board); //gets an array of valid locations 
        int [] Column = {3,4,2,5,1,6,0};// used to optimize alphabata pruning
        ...
        if (Player == maxPlayer){
            ...
            for  (int x = 0; x < game.getWidth(); x++ ){
                if (validLoc[Column[x]] == 1){
                    int y = tempRow(game, board, Column[x]);
                    ...
                    tempDrop(tempBoard, Column[x], y, maxPlayer);
                    int [] newScore = MiniMax(game, tempBoard, depth -1 , alpha, beta,  minPlayer);
                    if (newScore[0] > Max[0]) {
                        Max[0] = newScore[0];
                        Max[1] = Column[x];
                    }
                    //alphabeta 
                    if (alpha > newScore[0]) alpha = newScore[0];
                    if (beta <= alpha) break;
                }
            }
            return Max;
        ...
        //Mini
            ...
            for  (int x = 0; x < game.getWidth(); x++ ){
                if (validLoc[Column[x]] == 1){
                    ...
                    tempDrop(tempBoard, Column[x], y, minPlayer);
                    int newScore [] = MiniMax(game, tempBoard, depth -1 , alpha, beta, maxPlayer);
                    if (newScore[0] < Mini[0]) {
                        Mini[0] = newScore[0];
                        Mini[1] = Column[x];
                    }
                    //alphabeta
                    if (beta < newScore[0]) beta = newScore[0];
                    if (beta <= alpha) break;
                }
            }
            return Mini;
    ...
    private int eval_set (int [] set, int player){
        ...
        if (oppCount == 3 && emptyCount ==1) score -= 7; //-7 as alphabeta but -4 worked better for P1 but lost as P2
